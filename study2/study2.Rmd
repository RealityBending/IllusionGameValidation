...
# Drop also participant with bad block
df <- filter(
  df,
  !(Participant == "5c43cd414fe4f800016e4983_0zp37" & df$Block == 2))
...
formula <- brms::bf(
  Error ~ logmod(1 + Illusion_Difference) * logmod(1 + Illusion_Strength) +
    (1 | Participant),
  family = "bernoulli"
)
...
plot_descriptive_rt <- function(data, side = "leftright") {
  ...
  p1 <- data |>
    ...
    ggdist::stat_slabinterval(aes(fill = Illusion_Strength, group = Illusion_Strength, color = Illusion_Strength), alpha = 0.5, normalize = "groups", position="dodge") +
    ...
    scale_fill_gradientn(colours = c("#4CAF50", "#009688", "#00BCD4", "#2196F3", "#3F51B5", "#673AB7", "#9C27B0")) +
    coord_cartesian(ylim = c(125, 3000)) +
    theme_modern() +
    labs(
      color = "Illusion Strength",
      fill = "Illusion Strength",
  ...
  p2 <- data |>
    ...
    geom_vline(xintercept = 0, linetype = "dashed", alpha = 0.6) +
    ggdist::stat_slabinterval(aes(fill = Illusion_Difference, group = Illusion_Difference, color = Illusion_Difference), alpha = 0.5, normalize = "groups", position="dodge") +
    # geom_line(aes(color = Illusion_Difference), position = position_dodge(width=dodge2)) +
    scale_y_continuous(expand = c(0, 0)) +
    scale_x_continuous(expand = c(0, 0)) +
    scale_color_gradientn(colours = c("#F44336", "#FFC107", "#4CAF50")) +
...
# )
formula <- brms::bf(
  RT ~ t2(Illusion_Difference, Illusion_Strength, k=4) +
    (1 | Participant)
