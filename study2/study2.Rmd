...
dfsub$Participant[grepl("60be8d", dfsub$Participant)]
...
outliers <- c(
  ...
  "60dd626e212aa26c16d953b7_vw35k",
  # 2nd block bad, first block 1/3 bad
  # Prolific Status: RETURN REQUESTED (26/08)
  "5eed37c13661d91555c38308_hpgnt",
  # Prolific Status: RETURN REQUESTED (26/08)
  "6159f8a9a2d80ecf1a5acd30_zc8rt",
  # Prolific status: accepted (not enough proof)
  "6136aa846e80ebd7daf6e148_aggf6"
)
...
partial_outliers <- c(
  ...
  # Entire 2nd block bad
  "5ff8b21dd2ca6f12bdaa5be8_c5rs6",
  # Half of 2nd block bad
  "613a9193ed68e49209b471b7_wrymn",
  # 2nd block not good
  "60be8dd61cec210d627483de_cmfx4")
```
...
In the first part of the study focuses on modelling the effect of illusion strength and task difficulty, on errors and reaction time, within each illusion. In order to achieve that, we will start by fitting a General Additive Models (GAM), which can accommodate possible non-linear effects and interactions. Using them as a "ground-truth", we will attempt at approximating them using general linear models, in which we can specify a participant-level variability of the effects (via random effects).

We will then extract this participant-level variability in the effect of illusion strength, task difficulty and their interaction and use it as inter-individual features. We will estimate the relationship across different illusions using factor analysis (FA) and structural equation modelling (SEM). 


```{r make_GAMs, message=FALSE, warning=FALSE, eval=runModels}
for(ill in c("Delboeuf", 
  ...
  model <- brms::brm(
    brms::bf(
      ...
      sigma ~ t2(Illusion_Difference, Illusion_Strength, k=8, bs="cr") + (1 | Participant),
      beta ~ t2(Illusion_Difference, Illusion_Strength, k=8, bs="cr") + (1 | Participant)
    ),
...
formula <- brms::bf(
  RT ~ Illusion_Effect / (sqrt(Illusion_Difference) * sqrt(abs(Illusion_Strength)))  +
    (1 + Illusion_Effect / (sqrt(Illusion_Difference) * sqrt(abs(Illusion_Strength))) | Participant),
  sigma ~ Illusion_Effect / (sqrt(Illusion_Difference) * sqrt(abs(Illusion_Strength))) + (1 | Participant),
  beta ~ Illusion_Effect / (sqrt(Illusion_Difference) * sqrt(abs(Illusion_Strength))) + (1 | Participant),
  family = "exgaussian"
)
...
formula <- brms::bf(
  Error ~ Illusion_Effect / (sqrt(Illusion_Difference) * logmod(abs(Illusion_Strength))) +
...
formula <- brms::bf(
  RT ~ Illusion_Effect / (sqrt(Illusion_Difference) * logmod(abs(Illusion_Strength)))  +
    (1 + Illusion_Effect / (sqrt(Illusion_Difference) * logmod(abs(Illusion_Strength))) | Participant),
  sigma ~ Illusion_Effect / (sqrt(Illusion_Difference) * logmod(abs(Illusion_Strength))) + (1 | Participant),
  beta ~ Illusion_Effect / (sqrt(Illusion_Difference) * logmod(abs(Illusion_Strength))) + (1 | Participant),
  family = "exgaussian"
)
...
formula <- brms::bf(
  ...
  sigma ~ Illusion_Effect / (logmod(Illusion_Difference) * logmod(abs(Illusion_Strength))) + (1 | Participant),
  beta ~ Illusion_Effect / (logmod(Illusion_Difference) * logmod(abs(Illusion_Strength))) + (1 | Participant),
  family = "exgaussian"
)
