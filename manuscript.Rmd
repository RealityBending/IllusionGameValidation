...
This thereby opens the door to a standardizable experimental task in which participants can be asked to make perceptual judgments about the targets (e.g., which segment in the longest) under different conditions of objective difficulty and illusion strength. Moreover, the illusory effect can be set to either be "incongruent" (increasing the difficulty of the task by biasing the perception in the opposite way) or "congruent" (making the task easier). Although visual illusions are inherently tied to subjective perception, this framework radically reverses the traditional paradigm in favor of an approach that could quantify the "objective" effect of illusions by measuring its behavioral effect (error rate and reaction times) on the performance in a perceptual task.
...
We generated 56 stimuli for each of the 10 illusion types. These stimuli resulted from the combination of 8 linearly-spread levels levels of task difficulty (e.g., [1, 2, 3, 4, 5, 6, 7], where values correspond to the objective difference between targets, i.e., 1 corresponds to the smallest objective difference and thereby the highest level of difficulty in the task) and 7 levels of illusion strength (3 values (i.e, degrees) of strength on the side of illusion congruence, 3 on the side of illusion incongruence, and 0; e.g., [-3, -2, -1, 0, 1, 2, 3]. Notably, the magnitude of these values correspond to illusion strength (greater values represent greater strength) whereas the valence indicates the illusion congruence (negative sign represents an illusion congruent effect).

The 10 types of illusion stimuli were randomly presented in distinct blocks, within which the 56 stimuli were also displayed in a randomized order. After the first series of 10 blocks was presented, another series was displayed (with the same set of stimuli but with new randomized block and trial orders). All in all, each participant saw 56 different trials per illusion type (10 blocks in total), in 2 series of blocks (amounting to a total of 1120 trials). In each illusion trial, participants were required to evaluate the targets comprising the illusions without being distracted by its biasing contextual information. For example, in a Müller-Lyer illusion trial, participants were instructed to evaluate the length of line segments without being distracted by the direction in which the arrows at its ends are pointing towards. Importantly, participants were told to respond "as fast as possible without making errors" before each block of trials (i.e., an explicit double constraint incorporated to mitigate the inter-individual variability in the speed-accuracy trade off in responses). This task was implemented using *jsPsych* [@deleeuw2014]. For specific details of the instructions given for each illusion type, please refer to the experiment code.
...
The best models were $log(diff)*strength$ for Delboeuf; $sqrt(diff)*strength$ for Ebbinghaus; $log(diff)*log(strength)$ for Rod and Frame; $sqrt(diff)*sqrt(strength)$ for Vertical-Horizontal; .... For all of the specified models, the effect of illusion strength, task difficulty and their interaction were significant.
