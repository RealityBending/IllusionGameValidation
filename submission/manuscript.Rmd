...
The aim of the present preregistered study is three-fold. First, we will test this novel paradigm by investigating if the effect of illusion strength and task difficulty can be manipulated continuously for 10 different classic illusions (Delboeuf, Ebbinghaus, Rod and Frame, Vertical-Horizontal, Zöllner, White, Müller-Lyer, Ponzo, Poggendorff, Contrast). Next, we will investigate the latent factors structure of participant-level performance scores to these illusions and test the existence of a common factor of illusion sensitivity. Finally, we will explore how illusion sensitivity relates to demographic characteristics, contextual variables (pertaining to the experiment setting), and personality traits.
...
We excluded 6 participants upon inspection of the average error rate (when close to 50%, suggesting random answers), and reaction time distribution (when it was implausibly fast). For the remaining participants, we discarded blocks with more than 50% of errors (2.16% of trials), possibly indicating that instructions were misunderstood (e.g., participants focused on the shorter line instead of the longer one), and 0.76% trials with extreme response times (\< 125 ms or \> 4 SD above mean). Additionally, due to a technical issue, no personality data was recorded for the first eight participants.
...
The inter-individual variability in the effect of illusion strength and its interaction with task difficulty, were extracted and used as participant-level scores. We then explored the relationship of these indices across different illusions using exploratory factor analysis (EFA) and structural equation modelling (SEM), and tested the existence of a general of illusion sensitivity (Factor *i*).

Finally, for each of the individual illusion sensitivity scores (10 illusion-specific factors and the general Factor *i*), we tested the effect of contextual variables (screen size, screen refresh rate), demographic variables (sex, education, age) as well as personality traits. 
...
The parametric illusion generation framework developed in @makowski2021 proposes to conceptualize illusions as being composed of targets and distractors that can be manipulated independently and continuously. In the present study, we have shown that such gradual modulation of illusion strength is effectively possible across 10 different types of classic visual illusions. Specifically, increasing the illusion strength led to an increase in the likelihood of errors, as well as the average and spread of RTs (but only up to a point, after which participants become faster at responding with the wrong answer). As such, this important methodological step not only helps us move away from traditional illusion research paradigms which are highly dependent on participants' subjective experiences of illusory effects; opening the door for new illusions-based paradigms and tasks, it also allows us to study the effect of illusions under different conditions and to quantify illusion sensitivity using objective behavioral outcomes, such as accuracy or speed. For instance, using this novel framework accorded by *Pyllusion* to objectively manipulate the parameters of the Zöllner illusion, we were able to find an effect of illusion strength on errors that was distinct from the other 9 illusions examined. For a more detailed discussion on this specific illusion effect, please refer to the Supplementary Materials (Part 2 - Discussion). 
...
However, although the nature of the illusions were different in terms of the perceptual task underlying the illusory effect (contrast-based, size-estimation, angle-perception), the possibility of our general factor being driven by inter-individual perceptual skills variability (or other cognitive skills) cannot be discarded. Future studies should investigate the relationship between perceptual abilities (in a similar task, but without illusions) and general illusion sensitivity, and assess the psychometric properties of similar paradigms, including their stability (e.g., test-retest reliability) and validity.

<!-- Personality -->

Finally, we found the sensitivity to illusions to be positively associated with "positive" personality traits, such as *Agreeableness* and *Honesty-Humility*, and negatively associated with maladaptive traits such as *Antagonism*, *Psychoticism*, *Disinhibition* and *Negative Affect*. Although empirical evidence proposing significant links between illusion sensitivity and maladaptive personality traits are scarce, these results are consistent with past findings relating pathological egocentric beliefs (often observed alongside *Psychoticism*, @fox2006) to reduced context integration, manifesting in a tendency to separate objects from their surroundings when processing visual stimuli[ [@ohmann2016; @konrath2009seeing; @fox2006]. As such, the association between maladaptive traits and lower illusion sensitivity could be linked to a self-centered, decontextualized and disorganized information processing style. Conversely, the relationship between sensitivity to illusions and "positive" personality traits could be drawn from research relating cognitive styles (i.e., field dependence - a tendency to rely less on external cues in ambiguous contexts) to dispositional traits like hostility, aggression and narcissism [@zhang2017; @konrath2009seeing; @pessoa2008], which negatively correlate with "positive" traits like *Agreeableness* and *Honesty-Humility* (see Supplementary Materials, Discussion - Part 3 for an in-depth discussion on the relationship between personality traits, cognitive styles and illusion sensitivity). 

Importantly, such findings highlight the relevance of illusions beyond the field of visual perception, pointing towards an association with high-level domain-general mechanisms. In particular, the significant findings in favor of a relationship between maladaptive personality traits and illusion sensitivity could be viewed in relation to clinical findings, of which greater resistance to illusions have often been reported among patients with schizophrenia [@notredame2014; @pessoa2008; @grzeczkowski2018], especially in association with schizotypal traits such as cognitive disorganization [@cretenoud2019; @lanyi2022can]. While the search for the exact mechanism(s) underlying these links is an important goal of future research, our observations unlock the potential of illusion-based tasks as sensitive tools to capture specific inter-individual neuro-cognitive differences.
