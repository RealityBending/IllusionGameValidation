...
```{r message=FALSE, warning=FALSE, cache=cache,results='hide'}
best_models <- function(data) {
  models <- list()
  for(i in 1:2) {
    for(j in 1:2) {
      for(k1 in c("", "_log")) {  # c("", "_log")
        for(k2 in c("", "_log")) {  # c("", "_log")
          for(side in c("", "-side")) {
...
```{r message=FALSE, warning=FALSE, cache=cache}
std_params <- function(model, min=0, max=1) {
  estimate_relation(
  model,
...
```{r message=FALSE, warning=FALSE, cache=cache}
formula <- brms::bf(
  Error ~ Illusion_Side * Illusion_Difference * poly(Illusion_Strength, 2) + 
    ((1 + Illusion_Side + Illusion_Difference + Illusion_Strength) | Participant),
  family = "bernoulli"
)
